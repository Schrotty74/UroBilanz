...
def build_manual(language):
    ...
    story.extend(section(data["sections"]["first"], [
        ...
        first_steps,
        "Wenn macOS die native UroBilanz-App beim ersten Start blockiert: Die App einmal normal öffnen, dann Systemeinstellungen → Datenschutz & Sicherheit öffnen, zum Bereich Sicherheit scrollen und bei UroBilanz Dennoch öffnen wählen. Die Warnung mit Öffnen bestätigen und bei Bedarf authentifizieren. Dennoch öffnen erscheint nur für begrenzte Zeit nach dem blockierten Startversuch. Dadurch wird nur für diese App eine Ausnahme angelegt; Gatekeeper wird nicht systemweit deaktiviert. Die Freigabe nur für eine UroBilanz-App aus dem offiziellen GitHub-Release verwenden." if language == "de" else "If macOS blocks the native UroBilanz app on first launch: Open the app normally once, then open System Settings → Privacy & Security, scroll to Security, and choose Open Anyway for UroBilanz. Confirm the warning with Open and authenticate if macOS asks. Open Anyway is shown only for a limited time after the blocked launch attempt. This creates an exception only for this app and does not disable Gatekeeper system-wide. Use it only for a UroBilanz app obtained from the official GitHub release.",
