...
assert.match(webCoreSource, /const APP_VERSION = "1\.7\.4-beta\.1"/, "Zentrale Web-App-Version fehlt");
assert.match(webAppSource, /const appVersion = APP_VERSION/, "Web-App verwendet nicht die zentrale Version");
assert.match(webIndexSource, /id="openAbout"/, "Ueber-Ausloeser fehlt");
assert.match(webIndexSource, /discord\.gg\/RbsvqRCPQ/, "Discord-Link fehlt");
...
assert.match(webIndexSource, /data-ai-service="chatgpt"/, "ChatGPT-Erststart-Hilfe fehlt");
assert.match(webIndexSource, /data-ai-service="gemini"/, "Gemini-Erststart-Hilfe fehlt");
assert.match(webIndexSource, /data-ai-service="claude"/, "Claude-Erststart-Hilfe fehlt");
assert.match(webIndexSource, /\.\/assets\/ai-chatgpt-logo\.jpg/, "Lokales ChatGPT-Logo fehlt");
assert.match(webIndexSource, /\.\/assets\/ai-gemini-logo\.svg/, "Lokales Gemini-Logo fehlt");
assert.match(webIndexSource, /\.\/assets\/ai-claude-logo\.png/, "Lokales Claude-Logo fehlt");
assert.match(webAppSource, /chatgpt: "https:\/\/chatgpt\.com\/"/, "ChatGPT-URL fehlt");
assert.match(webAppSource, /gemini: "https:\/\/gemini\.google\.com\/app"/, "Gemini-URL fehlt");
assert.match(webAppSource, /claude: "https:\/\/claude\.ai\/new"/, "Claude-URL fehlt");
assert.match(webAppSource, /UroBilanz-Handbuch-DE\.pdf/, "Deutscher Handbuch-Link fehlt");
assert.match(webAppSource, /UroBilanz-User-Manual-EN\.pdf/, "Englischer Handbuch-Link fehlt");
assert.match(webAppSource, /github\.com\/Schrotty74\/UroBilanz\/blob\/main\/docs\/output\/pdf/, "KI-Hilfe muss die oeffentlichen PDF-Handbuchseiten verwenden");
assert.ok(fs.existsSync(path.join(__dirname, "..", "..", "..", "docs", "output", "pdf", "UroBilanz-Handbuch-DE.pdf")), "Deutsches Handbuch-PDF fehlt");
assert.ok(fs.existsSync(path.join(__dirname, "..", "..", "..", "docs", "output", "pdf", "UroBilanz-User-Manual-EN.pdf")), "Englisches Handbuch-PDF fehlt");
const firstStartPromptSource = webAppSource.match(/function firstStartHelpPrompt[\s\S]*?async function copyFirstStartPromptAndOpen/)?.[0] || "";
assert.doesNotMatch(firstStartPromptSource, /state\.rows|state\.days|rawCsv|localStorage|\/Users\//, "Erststart-Prompt darf keine Nutzerdaten lesen");
assert.match(firstStartPromptSource, /UroBilanz gerade zum ersten Mal geöffnet/, "Deutscher Erststart-Prompt fehlt");
assert.match(firstStartPromptSource, /just opened UroBilanz for the first time/, "Englischer Erststart-Prompt fehlt");
assert.match(firstStartPromptSource, /Messtaggrenze von 06:00 bis 05:59/, "Deutscher Erststart-Prompt muss die UroBilanz-Regeln erklaeren");
assert.match(firstStartPromptSource, /measurement-day boundary from 06:00 to 05:59/, "Englischer Erststart-Prompt muss die UroBilanz-Regeln erklaeren");
assert.match(firstStartPromptSource, /vollständige offizielle UroBilanz-Handbuch/, "Deutscher Prompt muss das vollstaendige Handbuch benennen");
assert.match(firstStartPromptSource, /complete official UroBilanz user manual/, "Englischer Prompt muss das vollstaendige Handbuch benennen");
assert.doesNotMatch(firstStartPromptSource, /README\.de\.md|README\.md|Projekt-README|project README/, "KI-Prompt darf keinen README-Link oder README-Verweis enthalten");
assert.ok(webAppSource.includes('window.confirm(t("first_start_copy_confirm", { service: firstStartHelp.serviceTitles[service] || service }))'), "KI-Hilfe muss das Kopieren und Oeffnen zuvor bestaetigen");
assert.match(webI18nDeSource, /first_start_copy_confirm:/, "Deutscher KI-Bestaetigungstext fehlt");
