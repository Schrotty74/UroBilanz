...
- Day view with local full-text search across visible types, individual and
  daily amounts, times, and notes, plus a urine, water, or note filter,
  without changing evaluations or exports.
...
- Web app and native SwiftUI app work locally without automatic data transfer
  to external servers. The web app can be installed as a PWA and starts
  without a connection after its first load.
...
Start `Start_Urinprotokoll.command` in `apps/web`. It starts a local server
and opens the installable PWA in a browser. After its first load, the browser
can install it as an app and open it without a connection. Opening
`index.html` directly remains supported, but does not provide PWA installation
or an offline cache.
...
If Gatekeeper blocks the UroBilanz macOS app on a current macOS version:

1. Open `UroBilanz.app` normally once so macOS registers the blocked launch.
2. Open **System Settings → Privacy & Security** and scroll to **Security**.
3. Choose **Open Anyway** for UroBilanz.
4. Confirm the warning with **Open** and authenticate if macOS asks you to.

The **Open Anyway** option is shown only for a limited time after the blocked launch attempt. Override Gatekeeper only for an UroBilanz build obtained from the official GitHub release. This creates an exception for that specific app and does not disable Gatekeeper system-wide.
...
the app or discussed on [Discord](https://discord.gg/Zy93AaYFaj).
